...
> - CHECKPOINT-23: Gate 2 (Phase 3 Exit) → **push + HANDOVER.md**
...
### FR Implementation Tasks (22 total)

#### FR-01: Create complete PostgreSQL schema with all core tables (`users`, `conversations`
**Task**: Create complete PostgreSQL schema with all core tables (`users`, `conversations`, `messages`, `knowledge_base`, `platform_configs`, `escalation_queue`, `user_feedback`, `security_logs`) plus indexe...
**SAD Mapping**:
- Module: `models`
- File:   `03-development/src/app/models.py`
**Forbidden**:
...
**Task**: Accept Telegram Bot API webhook POST requests, parse the platform-specific payload, and produce an immutable `UnifiedMessage` dataclass instance per SPEC.md §Unified Message Format. Must extract `p...
**SAD Mapping**:
- Module: `telegram_adapter`
- File:   `03-development/src/app/adapters/telegram_adapter.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Accept LINE Messaging API webhook POST requests, parse the platform-specific payload, and produce an immutable `UnifiedMessage` dataclass instance. Must extract `reply_token` from LINE events.
**SAD Mapping**:
- Module: `line_adapter`
- File:   `03-development/src/app/adapters/line_adapter.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Verify Telegram webhook request authenticity using HMAC-SHA256 over the secret key derived from `bot_token`. Reject with `AUTH_INVALID_SIGNATURE` (401) if verification fails. Per SPEC.md §Webhook S...
**SAD Mapping**:
- Module: `signature`
- File:   `03-development/src/app/security/signature.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Verify LINE webhook request authenticity using HMAC-SHA256 over `channel_secret` with Base64-encoded digest comparison. Reject with `AUTH_INVALID_SIGNATURE` (401) if verification fails. Per SPEC.md...
**SAD Mapping**:
- Module: `signature`
- File:   `03-development/src/app/security/signature.py`
**Forbidden**:
...
**Task**: Define and enforce the immutable `UnifiedMessage` dataclass with all required fields (`platform`, `platform_user_id`, `unified_user_id`, `message_type`, `content`, `raw_payload`, `received_at`, `re...
**SAD Mapping**:
- Module: `unified`
- File:   `03-development/src/app/adapters/unified.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Define the generic `ApiResponse[T]` and `PaginatedResponse[T]` dataclasses per SPEC.md §API Design — Unified Response Format. `ApiResponse` must carry `success`, `data`, `error`, `error_code`. `Pag...
**SAD Mapping**:
- Module: `unified`
- File:   `03-development/src/app/adapters/unified.py`
**Forbidden**:
...
**Task**: Normalize all inbound message text using Unicode NFKC normalization, strip non-printable characters (except `\n` and `\t`), and trim whitespace. Per SPEC.md §Security Layer — Input Sanitizer L2. Mu...
**SAD Mapping**:
- Module: `sanitizer`
- File:   `03-development/src/app/processing/sanitizer.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Detect and mask PII patterns in message text: phone numbers (Taiwan formats `\d{4}-\d{3,4}-\d{3,4}` and `\d{10,11}`), email addresses, and Taiwan-formatted addresses (city/county + road/street/lane...
**SAD Mapping**:
- Module: `pii_masker`
- File:   `03-development/src/app/processing/pii_masker.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Enforce per-platform per-user rate limiting using the Token Bucket algorithm. Default capacity and refill rate of 100 tokens/second. Requests exceeding the limit receive HTTP 429 with `RATE_LIMIT_E...
**SAD Mapping**:
- Module: `rate_limiter`
- File:   `03-development/src/app/security/rate_limiter.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Query `knowledge_base` table for matching answers using SQL `ILIKE` on question text and `= ANY(keywords)` array match. Return a `KnowledgeResult` with `source="rule"`, confidence 0.95 for exact su...
**SAD Mapping**:
- Module: `matcher`
- File:   `03-development/src/app/knowledge/matcher.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: When no rule match is found (Layer 1 returns None), create an escalation record in `escalation_queue` with `reason="no_rule_match"` or `"out_of_scope"`, priority=0 (normal), and return a `Knowledge...
**SAD Mapping**:
- Module: `escalation`
- File:   `03-development/src/app/knowledge/escalation.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Emit all log entries as single-line JSON objects with fields: `timestamp` (ISO 8601 UTC), `level` (DEBUG/INFO/WARN/ERROR/CRITICAL), `service` (configurable, default "omnibot"), `message`, and arbit...
**SAD Mapping**:
- Module: `logger`
- File:   `03-development/src/app/infrastructure/logger.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Expose `GET /api/v1/health` returning JSON `{"status": "healthy
**SAD Mapping**:
- Module: `health`
- File:   `03-development/src/app/infrastructure/health.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Provide a `docker-compose.yml` with services: `omnibot-api` (build from ., port 8000, healthcheck via `/api/v1/health`, depends on postgres+redis with `condition: service_healthy`), `postgres` (pgv...
**SAD Mapping**:
- Module: `yml`
- File:   `03-development/src/docker-compose/yml.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Provide SQL query scripts for Phase 1 ODD metrics: (a) FCR rate calculation over 30-day window for `scope_type='in_scope'` conversations with non-null `first_contact_resolution`, (b) average and p9...
**SAD Mapping**:
- Module: `queries/`
- File:   `03-development/src/queries/.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Define and use standardized error codes for Phase 1: `AUTH_INVALID_SIGNATURE` (401), `RATE_LIMIT_EXCEEDED` (429), `KNOWLEDGE_NOT_FOUND` (404), `VALIDATION_ERROR` (422), `INTERNAL_ERROR` (500). Each...
**SAD Mapping**:
- Module: `errors`
- File:   `03-development/src/app/infrastructure/errors.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: All Phase 1 Python code must follow constitution §4 naming conventions: `snake_case` for variables/functions, `PascalCase` for classes, `UPPER_SNAKE` for constants; all public functions must have d...
**SAD Mapping**:
- Module: `radon enforcement)`
- File:   `03-development/src/radon enforcement).py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Implement the core message processing pipeline that orchestrates each inbound request end-to-end: (1) IP Whitelist interception → (2) webhook signature verification → (3) platform adapter parse → (...
**SAD Mapping**:
- Module: `pipeline`
- File:   `03-development/src/app/processing/pipeline.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Define the immutable `UnifiedResponse` dataclass representing the system reply with fields: `platform`, `user_id`, `content` (the text to send back), `source` (KnowledgeSource enum: `rule`/`escalat...
**SAD Mapping**:
- Module: `unified`
- File:   `03-development/src/app/adapters/unified.py`
**Forbidden**:
...
**Task**: Load and manage configuration from environment variables and/or a `config.yaml` file: bot tokens (`TELEGRAM_BOT_TOKEN`, `LINE_CHANNEL_ACCESS_TOKEN`), channel secrets (`TELEGRAM_WEBHOOK_SECRET`, `LI...
**SAD Mapping**:
- Module: `config`
- File:   `03-development/src/app/infrastructure/config.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
**Task**: Implement IP Whitelist interception to filter out requests from unofficial IPs before computing HMAC signature. Unofficial IPs must be rejected with HTTP 403. Missing or empty IPs must be rejected ...
**SAD Mapping**:
- Module: `ip_whitelist`
- File:   `03-development/src/app/security/ip_whitelist.py`
**Forbidden**:
- app/infrastructure/ (deprecated)
- @covers: L1 Error
- @type: edge
...
> linting(90) · type_safety(85) · test_coverage(80) · security(80) · secrets_scanning(100) · license_compliance(100) · mutation_testing(70) · integration_coverage(60) · test_assertion_quality(60)  [D4 TEST_INVENTORY.yaml imperative check ≥60% · D4 backward spec-coverage ≥40%]
...
- [ ] **G2c** Finalize Gate 2:
  ...
  ```
- [ ] **[D4-BACKWARD]** D4 backward spec-coverage-check (Gate 2 threshold 40%):
  ```bash
...
- [ ] **[TDD-PRECHECK]** Verify TDD checks pass — advance-phase enforces all three:
  ...
  - `python3 harness_cli.py spec-coverage-check --project . --threshold 40.0` (exit 10)
  - `python3 harness_cli.py check-test-inventory --project . --threshold 60.0 --strict` (exit 12)
